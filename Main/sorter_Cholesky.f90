...
case(1) ! DALTON
   ...
   call readlbl(rdr%unit,'BASTWOEL')
...
case(2) ! MOLPRO
   ...
   if(sum(nt(1:nsk))/=NBas) then
      write(*,*) 'ERROR while checking AOTWOINT.mol!!!'
      stop
   endif
...
case default
   ...
   write(*,'(a)') 'Unknown AOSOURCE in open_AOReaderChol'
   stop
...
case(5) ! TREXIO
   ...
   write(*,'(a)') 'AOSOURCE=5 not ready in get_TR_AOReaderChol'
   stop

case default

   write(*,'(a)') 'Unknown AOSOURCE in get_TR_AOReaderChol'
   stop
...
do 
  ...
  if(ios<0) then
     write(*,*) 'ERROR!!! Empty section in AOTWOINT!'
     stop
  endif
  if(label(1)=='********') then
