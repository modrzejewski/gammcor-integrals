...
! -------------------------------------------------------------------------------------
! Marcin Modrzejewski, University of Warsaw
! -------------------------------------------------------------------------------------
...
contains
      ...
      subroutine chol_Rkab_ExternalBinary(S, Vecs, CA, a0, a1, CB, b0, b1, MaxBufferDimMB)
            !
            ...
            call chol_MOTransf_TwoStep(S, Vecs, CA, a0, a1, CB, b0, b1, MaxBufferDimMB)
      end subroutine chol_Rkab_ExternalBinary
            ...
            associate ( &
                  ...
                  do q = 1, NAO
                        do p = 1, q
                              ...
                              pq = pq + 1
                              if (p /= q) then
                                    call linalg_vwT_x(W(:, NVecs), NA, CAT(:, p), CBT(:, q), NA, NB, ONE)
                                    call linalg_vwT_x(W(:, NVecs), NA, CAT(:, q), CBT(:, p), NA, NB, ONE)
                              else
                                    call linalg_vwT_x(W(:, NVecs), NA, CAT(:, p), CBT(:, q), NA, NB, ONE)
                              end if
                              if (pq == NOrbPairs .or. NVecs == BufferDim) then
                                    ...
                                    !
                                    call linalg_abT_x(S, ldS, R(:, pq0:pq1), ldR, W, NA*NB, NCholesky, NA*NB, NVecs, alpha=ONE, beta=ONE)
                                    NVecs = 0
                              end if
                        end do
                  end do
      ...
      subroutine chol_MOTransf_Step1(W, Nk, NAO, CAT, a0k, a1k, a0, a1, Rpq, NCholesky)
            ...
            do q = 1, NAO
                  do p = 1, q
                        ...
                        !
                        if (p /= q) then
                              call linalg_vwT_x(W(:, :, p), NCholesky, Rpq(:, pq), CAT(a0k:a1k, q), NCholesky, Nk, ONE)
                              call linalg_vwT_x(W(:, :, q), NCholesky, Rpq(:, pq), CAT(a0k:a1k, p), NCholesky, Nk, ONE)
                        else
                              call linalg_vwT_x(W(:, :, p), NCholesky, Rpq(:, pq), CAT(a0k:a1k, q), NCholesky, Nk, ONE)
                        end if
                  end do
      ...
      subroutine chol_MainLoop(R, NCholesky, D, Significant, SignificantDim, NSignificant, PrescreenError, &
            ...
            call msg("Dimensions of the Coulomb matrix: (" // str(NOrbPairs(BASE_PAIRS)) // "," // str(NOrbPairs(BASE_PAIRS)) // ")")
            ...
            call msg("Dimensions of 2e integral storage buffer: (" // str(NOrbPairs(BASE_PAIRS)) // "," // str(MaxNQualified*MaxBatchDim) // ")")
